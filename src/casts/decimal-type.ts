...
import {ScalarRangeOpt} from "@leyyo/scalar";
...
@Fqn(...FQN_NAME)
@AssignCast('Decimal')
export class DecimalType extends AbstractFormat<_T, _O> {
    ...
    cast(value: unknown, opt?: _O): _T {
        ...
        return this._scalar.float.ly_validate(num, opt);
    }
    docCast(target: unknown, property: PropertyKey, openApi: RecLike, opt?: _O): CastApiDocResponse {
        return this._scalar.float.ly_apiDoc(target, property, openApi, {}, opt);
    }
}